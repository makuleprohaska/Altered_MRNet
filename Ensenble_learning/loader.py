...
STDDEV = [0.229, 0.224, 0.225]  # ImageNet std
...
class MRDataset(data.Dataset):
    ...
    def weighted_loss(self, prediction, target, train):
        indices = target.squeeze(1).long()  # [B]
        weights_tensor = torch.tensor(self.weights, device=self.device, dtype=torch.float32)[indices].unsqueeze(1)  # [B, 1]
...
def load_data3(device, data_dir, labels_csv, batch_size=1, label_smoothing=0.1):
    ...
    train_dataset = MRDataset(data_dir, train_files, labels_dict, device, label_smoothing)
    valid_dataset = MRDataset(data_dir, valid_files, labels_dict, device, label_smoothing)
...
def load_data_test(device, data_dir, labels_csv, batch_size=1, label_smoothing=0.1):
    ...
    test_files.sort()
    test_dataset = MRDataset(data_dir, test_files, labels_dict, device, label_smoothing)
